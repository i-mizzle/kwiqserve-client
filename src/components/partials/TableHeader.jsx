...
const TableHeader = () => {
    ...
    return (
        <header className='px-8 lg:px-24 xl:px-32 2xl:px-44 w-full flex items-center justify-between py-5 fixed bg-white z-99'>
            <div className='flex items-center gap-x[10px] w-2/3 gap-x-5'>
                <Link to={`/storefront`} className='text-lg xl:text-xl font-family-bricolage-grotesque! font-bold'>{business.name}</Link>
            </div>
            
            {cartSelector?.fetchingCart ? <InlinePreloader /> : <Link to={`/cart`} className='w-10 h-10 rounded-full bg-ss-pale-blue flex items-center justify-center relative'>
