...
import { bulkCreateTable, createTable } from '../../../store/actions/tablesActions'
import NumberField from '../form/NumberField'

const MultipleNewTables = ({close}) => {
    ...
    const validateForm = () => {
        ...
        if(!tablePayload.quantity || tablePayload.quantity === '' || tablePayload.quantity < 1){
    ...
    const pushTable = () => {
        ...
        dispatch(bulkCreateTable(tablePayload))
    }
        ...
        <div className='w-full'>
            ...
            <div className='my-2'>
                <NumberField
                    ...
                    fieldId="table-qty"
                    inputLabel="Quantity" 
                    inputPlaceholder={`How many tables do you need to create?`}
                    preloadValue={tablePayload.quantity || ''}
                    hasError={validationErrors.quantity} 
                    returnFieldValue={(value)=>{setTablePayload({...tablePayload, ...{quantity: value}})}}
                    // helperText={`A serial number will be appended to the name provided here.`}
                />
            </div>
            <div className='my-2'>
                <TextField
            ...
            <div className='my-2'>
                <TextField
                    ...
                    helperText={`Create a short memorable code to easily refer to the table. A serial number will be appended to the code provided here`}
                />
            </div>
            <div className='my-2'>
                <TextField
                    ...
                    helperText={`The description will be added to all tables created`}
                />
            </div>
            <div className='my-2'>
            ...
            <div className="mt-8 flex flex-row-reverse gap-x-4">
                <button
                    ...
                    onClick={()=>{pushTable()}}
                >
                    {tablesSelector.creatingTable ? <InlinePreloader /> : 'Create tables'}
                </button>
...
export default MultipleNewTables