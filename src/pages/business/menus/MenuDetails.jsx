  ...
  useEffect(() => {
    const fetchMenu = async () => {
      try {
        ...
        const response = await axios.get(`${baseUrl}/menus/${menuId}`, { headers })
    ...
    if(menusState.deletedMenu !== null){
        dispatch( clearDeletedMenu())
        dispatch({
            type: SET_SUCCESS,
    ...
    if(menusState.updatedMenu !== null){
      dispatch( clearUpdatedMenu())
      dispatch({
        type: SET_SUCCESS,
    ...
    // fetchDocuments();
    dispatch(fetchItems('type=sale'))
    // dispatch(pullSingleItem('menu', menuId))

    return () => {
    
    };
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [refresh, menusState.deletedMenu, menusState.updatedMenu, dispatch, menuId]);
    ...
    const selectMenuItem = (itemIndex, newItem) => {
      ...
      // tempMenuDetails.items[itemIndex].parentItemCategory = newItem.parentItemCategory
    ...
    const updateMenuDetails = async () => {
      ...
      dispatch(updateMenu(menuId, menuDetails))
    }

    const [showDeleteConfirmation, setShowDeleteConfirmation] = useState(false);
    const [deleteConfirmationMessage, setDeleteConfirmationMessage] = useState(null);
    const deleteItem = async () => {
      setDeleteConfirmationMessage(`You are about to permanently delete this price card "${menuDetails.name}". This action cannot be reversed. Please confirm.`)
      setTimeout(() => {
      ...
      {!loaded ? 
        ...
        <div className='min-h-screen h-inherit py-2'>
          {itemsState.items?.items?.length > 0 ? <div className='w-full lg:w-10/12 xl:w-6/12 mx-auto p-1 bg-white '>
              <h3 className='text-3xl text-gray-800 font-bold'>Menu Details</h3>
              <p className='text-sm text-gray-600 mt-1 mb-4'>You can edit the menu by changing any of the details below or turn an item stock on/off. I an item's stock is off, it means users will not be able to order it.</p>
              <div>
                  <div className='my-2'>
                    <TextField
                      inputType="text" 
